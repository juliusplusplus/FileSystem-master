...
extern vector < vector<UFD> > FileInfo;//用户文件目录
extern UFD FileInput;
extern vector<Cluster> FileCluster;
extern vector<MFD> UsrInfo;//主文件目录
extern vector< vector<UOF> > FileState;//用户打开文件表
...
void do_Logout()
{
	if (curID == 0)cout << "默认管理员账号无法注销! " << endl;
	else {
		curID = 0;
		cout << "注销成功，请选择用户登陆!" << endl; return;
	}}
void do_register()
{
	...
	{
		int flag = 1;//判断重名
		if (strlen(cmd_in.cmd_num[1].c_str()) > 14) { cout << "用户名超过14个字符。" << endl; return; }
		if (strlen(cmd_in.cmd_num[2].c_str()) > 14) { cout << "密码超过14个字符。" << endl; return; }
		for (int i = 0; i < UsrInfo.size(); i++)
		{
